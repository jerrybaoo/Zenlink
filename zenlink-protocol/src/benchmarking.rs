...
benchmarks! {
	...
	bootstrap_create {
		let reward: Vec<<T as Config>::AssetId> = vec![ASSET_0.into()];
		let reward_amounts: Vec<(<T as Config>::AssetId, u128)> = vec![(ASSET_1.into(), 0)];
	...
	swap_exact_assets_for_assets{
		...
		let path: Vec<<T as Config>::AssetId> = vec![ASSET_0.into(), ASSET_1.into(), ASSET_2.into()];
